  ...
  class Config
    class << self
      ...
      def configure(config)
        ...
        unless DeploymentPlan::SubnetDistribution::ALLOWED.include?(@dynamic_subnet_strategy)
          raise ArgumentError, "Invalid dynamic_subnet_strategy '#{@dynamic_subnet_strategy}'; " \
            "valid values: #{DeploymentPlan::SubnetDistribution::ALLOWED.join(', ')}"
        end
